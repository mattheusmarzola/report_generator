defmodule ReportsGeneratorTest do
  ...
  describe "fetch_higher_cost/2" do 
    test "when the option is 'users', returns the user who spent the most" do 
      ...
      response = 
        ...
        |> ReportsGenerator.fetch_higher_corst("users")
    ...
    test "when the option is 'foods', returns the most consumed food" do 
      ...
      response = 
        file_name 
        |> ReportsGenerator.build()
        |> ReportsGenerator.fetch_higher_corst("foods")
    ...
    test "when an invalid option is given, returns an error" do 
      ...
      response = 
        file_name 
        |> ReportsGenerator.build()
        |> ReportsGenerator.fetch_higher_corst("invalid")
  ...
  describe "build_from_many/1" do
    test "when a file list is provided, builds the report" do
      ...
      {:ok, response} = ReportsGenerator.build_from_many(file_names)
    ...
    test "when a file list is not provide, returns a error" do 
      response = ReportsGenerator.build_from_many("file_error")
